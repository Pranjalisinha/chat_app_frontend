...
export default function ChatAI() {
  ...
  useEffect(() => {
    ...
    Cookies.get("token");
    const conversationResponse = async () => {
      try {
        const response = await axios.get(
          `${BackendUrl}/api/conversations/${userId}`
        );
  ...
  return (
    <div className="h-full grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-6">
      ...
      <div className="md:col-span-4 lg:col-span-3">
        ...
        <div
          className={`container-card p-4 md:block ${
      ...
      <div className="md:col-span-8 lg:col-span-9 container-card p-0 flex flex-col min-h-[60vh]">
        ...
        <div className="border-t border-white/50 dark:border-white/10 p-3">
          <div className="flex items-end gap-2">
            ...
            <button
              onClick={sendMessage}
              className="inline-flex items-center justify-center rounded-md bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 font-medium shadow-sm transition-colors"
            >
              Send
            </button>
          </div>
